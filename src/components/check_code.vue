...
<script>
    import { get_checkCode } from "../api/api";
    export default {
        name: 'SIdentify',
        props:{ 
            identifyPropCode:{
        ...
        methods: {
            ...
            //     let that = this;
            //     let para = {};
            //     get_checkCode(para).then(rest=>{
            //         if(rest.data.code + '' === '0'){
            //             that.identifyCode = rest.data.result.code;
            //             this.drawPic();
            //             this.$emit('listenChecCodeKey',rest.data.result.key)
            //         } else {
            //             this.$message({  showClose: true, message:  rest.data.message || "网络异常，请检查网络", type: "error" });
            //         }
            //     })
            // },
            // refreshCode(){
        ...
        mounted() {
            this.identifyCode = this.identifyPropCode;
            this.drawLine();
